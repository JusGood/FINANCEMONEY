...
const Dashboard: React.FC<Props> = ({ transactions, ownerFilter, onConfirmSale }) => {
  ...
  // CALCUL STATS ULTRA-PRÉCIS (Restauration logique Virement & Flip)
  const stats = useMemo(() => filtered.reduce((acc, curr) => {
    ...
    if (ownerFilter === Owner.GLOBAL) {
      if (curr.type === TransactionType.TRANSFER) return acc; // Transfert interne = neutre en Global
      ...
      // Vue individuelle : les virements impactent le solde
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-2 bg-slate-900 dark:bg-indigo-900 p-8 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden group transition-all duration-500">
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-3">
              <p className="text-[11px] font-black uppercase tracking-[0.4em] text-white/50 italic">VAULT : VALEUR RÉELLE</p>
            ...
            <div className="mt-8 flex gap-8 border-t border-white/10 pt-6">
               ...
               <div>
                  <p className="text-[10px] font-black text-white/30 uppercase mb-1">CRYPTO HODL</p>
            ...
            {showDetails && (
              <div className="mt-6 p-4 bg-black/20 rounded-2xl animate-in fade-in slide-in-from-top-2">
                 <p className="text-[9px] font-black text-white/40 uppercase mb-3 tracking-widest italic">Portefeuille Actif</p>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between shadow-sm transition-all hover:border-indigo-500/50">
           <div className="flex justify-between items-center mb-4">
             <h3 className="text-[11px] font-black tracking-[0.3em] uppercase text-slate-400 italic">Analyse de Performance IA</h3>
             <button onClick={fetchAiReport} disabled={loadingReport} className="text-[10px] font-black text-indigo-500 uppercase px-3 py-1 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg tracking-widest transition-all">Générer</button>
           </div>
           <div className="min-h-[60px] flex items-center">
             {aiReport ? (
               ...
               <div className="flex items-center gap-3">
                 ...
                 <span className="text-[11px] font-black uppercase text-slate-300 italic tracking-widest animate-pulse">Consultation du conseiller...</span>
               </div>
             )}
           </div>
        </div>
      ...
      {/* RÉSULTATS EN ATTENTE : RESTAURATION COMPLÈTE (Command Center) */}
      {pendingItems.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl mt-8">
           <div className="px-8 py-5 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
             <span className="text-[11px] font-black uppercase text-slate-400 tracking-[0.3em] italic">Audits en cours (Hors-Caisse) : {pendingItems.length}</span>
           </div>
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 divide-x divide-y divide-slate-100 dark:divide-slate-800">
             {pendingItems.map(p => (
               <div key={p.id} className="p-7 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all group flex flex-col justify-between min-h-[220px]">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <span className={`text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider ${p.type === TransactionType.INVESTMENT ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/40' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/40'}`}>
                        {p.type === TransactionType.INVESTMENT ? 'Stock Flip' : 'Comm Client'}
                      </span>
                      <div className="text-right">
                    ...
                    {p.client && <p className="text-[9px] font-bold text-slate-400 uppercase mb-3 italic tracking-wide">Client: {p.client}</p>}
                  ...
                  <button onClick={() => onConfirmSale(p.id)} className="w-full text-[10px] font-black uppercase tracking-[0.2em] bg-slate-900 dark:bg-indigo-600 text-white py-3 rounded-2xl hover:bg-emerald-500 transition-all shadow-lg active:scale-95 shadow-indigo-900/10">Clôturer Audit</button>
               </div>
             ))}
           </div>
        </div>
