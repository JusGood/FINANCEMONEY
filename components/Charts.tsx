...
export const BalanceTrendChart: React.FC<Props> = ({ transactions, ownerFilter = Owner.GLOBAL }) => {
  ...
  return (
    <div className="h-full w-full min-h-[160px]" style={{ minHeight: '160px' }}>
      <ResponsiveContainer width="100%" height="100%" minHeight={160}>
        <AreaChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
          ...
          <Tooltip 
            contentStyle={{ backgroundColor: '#020617', border: 'none', borderRadius: '12px', fontSize: '10px', color: '#fff', fontWeight: 'bold', padding: '10px' }}
            formatter={(value: any) => [`${(value || 0).toLocaleString('fr-FR')} €`, 'Solde']}
          />
          <Area type="monotone" dataKey="real" stroke="#6366f1" fill="#6366f1" fillOpacity={0.1} strokeWidth={2.5} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
export const CategoryPieChart: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
  ...
  if (categoryData.length === 0) return <div className="h-full flex items-center justify-center text-slate-400 italic text-[9px] font-black uppercase tracking-widest">Aucune donnée...</div>;
    ...
    <div className="h-full w-full min-h-[160px]" style={{ minHeight: '160px' }}>
      <ResponsiveContainer width="100%" height="100%" minHeight={160}>
        <PieChart>
          ...
          <Tooltip contentStyle={{ fontSize: '10px', borderRadius: '12px', fontWeight: 'bold' }} formatter={(value: any) => `${(value || 0).toLocaleString('fr-FR')} €`} />
        </PieChart>
      </ResponsiveContainer>
    </div>
  );
